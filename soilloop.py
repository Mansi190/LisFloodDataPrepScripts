...
def soilColumnsWaterBalance(index_landuse_all, is_irrigated, is_paddy_irrig, paddy_inactive, DtDay,
    ...
    for veg in range(num_vegs):   
        ...
        for q in prange(sim_pixels.size):
            ...
            RelSat1 = min(W1[veg,pix] / WS1[landuse,pix], 1.0) if PoreSpaceNotZero1a[landuse,pix] and WS1[landuse,pix] > 1e-12 else 0.0
            ...
            CourantTopToSubA = 0. if AvailableWater1a[q] <= 1e-12 else KUnSat1a[q] * DtDay / AvailableWater1a[q]
            CourantTopToSubB = 0. if AvailableWater1b[q] <= 1e-12 else KUnSat1b[q] * DtDay / AvailableWater1b[q]
            CourantSubToGW = 0. if AvailableWater2[q] <= 1e-12 else KUnSat2[q] * DtDay / AvailableWater2[q]
            ...
            NoSubS[q] = max(1, np.ceil(CourantSoil / max(CourantCrit, 1e-12)))
...
def saturationDegree(w, PoreSpaceNotZero, WRes, WS):
    if PoreSpaceNotZero and (WS - WRes) > 1e-12:
...
def __thetaFun(W, SoilDepth, PoreSpaceNotZero):
    return W / SoilDepth if PoreSpaceNotZero and SoilDepth > 1e-12 else 0.
...
def __satFun(W, WWP, WFC):
    return (W - WWP) / (WFC - WWP) if WFC > WWP else 0.0
