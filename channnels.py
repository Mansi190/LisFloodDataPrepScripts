...
from lisflood_utils import (GridInfo, log, check_imports, make_dirs,
                            gdal_convert_pcraster, load_grid, save_aligned, reproject_to_grid,
                            init_ee)
...
def convert_to_pcraster(tif_paths: dict) -> dict:
    log("STEP 4 - Converting GeoTIFFs -> PCRaster .map", "STEP")
    map_paths = {}
    VSTYPE = {
        "chan":     "VS_BOOLEAN",
        "changrad": "VS_SCALAR",
    ...
    for name, tif_path in tif_paths.items():
        ...
        vs       = VSTYPE.get(name, "VS_SCALAR")
        if gdal_convert_pcraster(tif_path, map_path, vs):
            map_paths[name] = map_path
            log(f"  {name}.map  V")
        else:
            log(f"  {name}.tif  V   (.map conversion failed - see manual_convert.sh)", "WARN")
...
def visualize(chan, changrad, chanbw, chanbnkf, chanman, chanleng, mask, info):
    ...
    try:
        ...
        panel(axes[0, 3], chanleng, "chanleng.map\n[m]",  "Purples", "m")
...
def print_summary(tif_paths: dict, map_paths: dict, info: GridInfo):
    ...
    for key, ini_name in ini_vars.items():
        val = map_paths.get(key, f"W  run manual_convert.sh for {key}.map")
...
def compute_and_download_gee_channels(info):
    ...
    # 1) Channel Mask from 30m Drainage Lines
    ...
    chanLength = ee.Image.constant(30.0).multiply(1.1).rename('chanleng')
    
    combined = ee.Image([
        channelMask.toFloat(), chanGrad.toFloat(), chanMan.toFloat(),
...
def process_local_channels(raw_tif, mask_arr, info):
    ...
    # 'chanbnkf' and 'chanbw' are removed from GEE bands, they will be computed locally
    band_names = ['chan', 'changrad', 'chanman', 'chanleng', 'chans']
    aligned_bands = {}
    ...
    with rasterio.open(raw_tif) as src:
        ...
        for i, name in enumerate(band_names):
            ...
            aligned = reproject_to_grid(
                ...
                src_nodata=np.nan,
                dst_nodata=-9999,
                resampling_method="nearest" if name == 'chan' else "bilinear"
            ).astype(np.float32)
    ...
    area_km2 = acc_arr * 0.0009 # 30m pixel is 900m^2 = 0.0009 km^2
    ...
    chan_raw = aligned_bands['chan']
    chan = np.where((chan_raw > 0.5) & (mask_arr > 0), 1, 0).astype(np.uint8)
